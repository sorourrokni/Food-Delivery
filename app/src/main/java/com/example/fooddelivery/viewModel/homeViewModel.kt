...
class homeViewModel(
    ...
    ,private val addressDao:AddressDao): ViewModel() {
    ...
    fun addToCart(name:String,number:Int=1){
        ...
        if (orderList.isEmpty()){
            ...
            if (allOrders.isEmpty()){
                val order=Order(1,food.price,payment_method.Direct_pay,Delivery.PickUp,address.id,email,orderStatus.TODO)
                orderDao.upsertOrder(order)
            }
            else{
                val order=Order(allOrders.last().id+1,food.price,payment_method.Direct_pay,Delivery.PickUp,address.id,email,orderStatus.TODO)
                orderDao.upsertOrder(order)
            }
